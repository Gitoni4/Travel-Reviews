...
app.get('/getLocations', async (req, res) => {
    ...
    console.log(params)
    let query = params['query'].split(' ')[0] + '+' + params['query'].split(' ')[1]
    query = query.latinize()

    try {
      const { data } = await axios.get(`https://maps.googleapis.com/maps/api/place/textsearch/json?query=${query}&type=${'' + params['type']}&key=AIzaSyBmTFtKqcgCcEhQCG7_WM8ikF2AQUiBOeg`)
      console.log(data['results'].length)
      let listOfPlaces = []
      for (let i = 0; i < data['results'].length; i++) {   
        console.log(data['results'][i].place_id)    
        let placeID = '' +  data['results'][i].place_id
        const placeDetails = await axios.get('https://maps.googleapis.com/maps/api/place/details/json?place_id=' + placeID + '&key=AIzaSyBmTFtKqcgCcEhQCG7_WM8ikF2AQUiBOeg')
        listOfPlaces.push(placeDetails['data'])
      }
      res.end(JSON.stringify(listOfPlaces))
    } catch (err) {
      //console.log(err)
